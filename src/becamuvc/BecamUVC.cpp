 ...
 * @implements 实现获取设备列表
 */
StatusCode BecamUVC::GetDeviceList(GetDeviceListReply& reply) {
	// 执行设备列表获取
	return BecamuvcDeviceHelper::GetDeviceList(reply.deviceInfoList, reply.deviceInfoListSize);
}

/**
 * @implements 实现释放设备列表
 */
void BecamUVC::FreeDeviceList(GetDeviceListReply& input) {
	// 执行释放
 ...
 */
StatusCode BecamUVC::GetDeviceConfigList(const std::string& devicePath, GetDeviceConfigListReply& reply) {
	// // 加个锁先
	// std::unique_lock<std::mutex> lock(this->mtx);
 ...
 */
void BecamUVC::FreeDeviceConfigList(GetDeviceConfigListReply& input) {
	// 执行释放
 ...
 * @implements 实现打开指定设备
 */
StatusCode BecamUVC::OpenDevice(const std::string& devicePath, const VideoFrameInfo& frameInfo) {
	// // 检查参数
	// if (devicePath.empty()) {
	// 	return StatusCode::STATUS_CODE_ERR_INPUT_PARAM;
	// }
 ...
 * @implements 实现获取视频帧
 */
StatusCode BecamUVC::GetFrame(uint8_t*& data, size_t& size) {
	// 加个锁先
	std::unique_lock<std::mutex> lock(this->mtx);
 ...
 * @implements 实现释放视频帧
 */
void BecamUVC::FreeFrame(uint8_t*& data) {
	// 释放获取到的帧
